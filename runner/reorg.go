...
func detectReorg(client *rpcclient.Client, db *dao.DB, block *wire.MsgBlock, height uint32) error {
	bitcoindPrevBlockHash := block.Header.PrevBlock.String()
	if height == 0 {
		return nil
	}
	indexPreBlockHash, err := db.BlockHash(height - 1)
	...
	for depth := uint32(1); depth < maxRecoverableReorgDepth; depth++ {
		...
		indexBlockHash, err := db.BlockHash(height - depth)
		if err != nil {
			return err
		}
		bitcoindBlockHash, err := client.GetBlockHash(int64(height - depth))
		if err != nil {
			return err
		}
...
func updateSavePoints(client *rpcclient.Client, db *dao.DB, height uint32) error {
	chainInfo, err := client.GetBlockChainInfo()
	if err != nil {
		return err
	}
	savepoints, err := db.ListSavepoint()
	if err != nil {
		return err
	}
	...
	if (height < savepointInterval || height%savepointInterval == 0) &&
		...
		if len(savepoints) >= int(maxSavepoint) {
			...
			if err := db.Delete(&tables.SavePoint{
				Id: savepoints[0].Id,
			}).Error; err != nil {
				return err
			}
			if err := db.Where("id<=?", savepoints[1].UndoLogId).
				Delete(&tables.UndoLog{}).Error; err != nil {
				return err
			}
		}
		latest := &tables.UndoLog{}
		if err := db.Last(latest).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
			return err
		}
		log.Log.Infof("creating savepoint at height %d", height)
		return db.Create(&tables.SavePoint{
			Height:    height,
			UndoLogId: latest.Id,
		}).Error
	}
	if len(savepoints) == 0 {
		if err := db.DeleteUndoLog(); err != nil {
			return err
		}
	}
	return nil
...
// The function then returns nil.
func handleReorg(db *dao.DB, height, depth uint32) error {
	log.Log.Infof("rolling back database after reorg of depth %d at height %d", depth, height)
	if err := db.Transaction(func(tx *dao.DB) error {
		...
		for rows.Next() {
			...
			if err := db.ScanRows(rows, &undoLog); err != nil {
				return err
			}
	...
	indexHeight, err := db.BlockCount()
	if err != nil {
		return err
	}
	log.Log.Infof("successfully rolled back database to height %d", indexHeight)
	return nil
}
